...
Lista *cria_lista(int tam_inicial){
    ...
    if(li == NULL){
        ...
        li->tam_inicial = tam_inicial;
        li->dados = malloc(tam_inicial * sizeof(struct aluno));
...
int insere_lista_ordenada(Lista *li, struct aluno a){
    ...
    else if(li->qtd == li->tam_inicial){

        li->dados = realloc(li->dados, li->qtd * sizeof(struct aluno));
    }
    int i=0;
...
int insere_lista_fim(Lista *li, struct aluno a){
    if(li == NULL){
        ...
        li->dados = realloc(li->dados, (li->qtd + li->tam_inicial) * sizeof(struct aluno));
    }
    else{
    li->dados[li->qtd] = a;
    li->qtd++;
...
int compactar_lista(Lista *li){
    ...
    float novo_tam = ceil(li->qtd/li->tam_inicial) * li->tam_inicial;
    li->dados = realloc(li->dados, novo_tam * sizeof(struct aluno));
    li->qtd = novo_tam;
    return 0;
    }
}