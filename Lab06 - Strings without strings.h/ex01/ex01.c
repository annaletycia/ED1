#include <stdio.h>
   ...
   int main()
   {
     ...
     for (i = 32; i < 127; i=i+7) //loop para limitar os valores de 32 ao 126
      {
          for (j = 0; j < 7; j++) //loop para limitar sete valores por linha
          {
              k = i + j; //armazena os valores na variavel k
             printf("%3d %c  ", k, k); //padronizando a saida para tres caracteres representando o valor numerico
             if(k == 126){
             break; 
          }
          }
